# JoshAnswers — Core Rules, Phrases & Market Intelligence
> Compiled from The Trading Fraternity (YouTube / StockMarketLive) + Meet Kevin (YouTube / mketf.com) research sweep.  
> Additional reference: Warren Buffett / Berkshire Hathaway letters, Morgan Stanley 2026 outlook, Wedbush equity research.  
> All content is educational. Nothing here is personalized financial advice.  
> Phrases marked *[direct]* are verified sourced quotes. Others are in the spirit of documented teaching.

---

## KNOWLEDGE EVOLUTION LOG — May 24, 2026
*"A good trader doesn't just follow one voice. He studies them all, decides what's real, and builds his own edge."*

### The Research Sweep
After deep research into **Meet Kevin** (Kevin Paffrath — @MeetKevin on YouTube, meetkevin.com),
cross-referenced against Josh's Trading Fraternity framework and reputable institutional sources
(Wedbush, Morgan Stanley, J.P. Morgan 2026 outlook), here is the full synthesis of how my
philosophy evolved — what I kept, what I cut, and what I built from the collision of the two.

---

### WHO IS MEET KEVIN

- **Real name:** Kevin Paffrath — licensed real estate broker and financial advisor
- **Platform:** YouTube (@MeetKevin, ~5M+ subs), podcast (The Meet Kevin Show), paid courses (joinmeetkevin.com)
- **Background:** Real estate investor turned stock market educator; built ~$30M net worth across YouTube revenue, real estate, stocks, and courses
- **ETF:** Launched The Meet Kevin Pricing Power ETF (NYSE: PP) in November 2022 — **closed and liquidated February 28, 2025** (underperformed, delisted)
- **Track record:** 33% pick accuracy tracked by TrueAlpha — good ideas, inconsistent execution
- **Style:** Fast-paced, macro-aware, growth/tech-heavy, high-conviction concentrated bets, fundamental analysis, willing to go speculative
- **Known picks:** Tesla (large position in ETF), NVDA, Enphase, Lemonade (LMND), HUT Mining, Shift Technologies

---

### MEET KEVIN'S CORE FRAMEWORK

#### The Pricing Power Filter
> *A company has pricing power when it can raise prices without losing customers.*

This is the Buffett moat concept applied to growth stocks:
- **What qualifies:** Enterprise software lock-in (ORCL), AI chips with no real alternative (AMD, NVDA), AI-native disruption (LMND)
- **What fails the test:** Commodity businesses, price-takers, declining brands with no differentiation

#### Kevin's Rules (synthesized from ETF prospectus, YouTube content, course material)
1. **Innovation is the entry point** — only invest in companies creating something new or disrupting an existing market
2. **Pricing power is the filter** — innovation without pricing power is just hype; you need both
3. **Macro context matters** — "Don't fight the Fed"; rate cycles dictate what kind of stocks work
4. **Fundamentals over narratives** — FCF positive or credible path to it; EPS growth; moat widening
5. **Concentration in conviction** — Kevin runs 50%+ in his top 3 names when he believes in them
6. **Macro-hedging** — his ETF could allocate up to 30% to targeted ETFs to hedge rate/geopolitical risk
7. **Long-term capital appreciation** — not a day-trader; buys and holds through volatility if thesis holds

#### Kevin's Blind Spots (what research revealed)
- **Over-concentration risk** — 50% in 3 stocks (TSLA, ENPH, NVDA) contributed to ETF failure
- **33% pick accuracy** — good at identifying themes, inconsistent on timing and entry
- **Ignores trading mechanics** — no stop-loss discipline, no position-sizing rules, no Ghetto Spread equivalent
- **Chases narratives** — HUT Mining, Shift Tech (speculative without rigorous catalyst framework)
- **ETF closure** — the PP ETF ultimately failed to sustain; execution gap exists between philosophy and results

---

### COLLISION ANALYSIS — JOSH vs. KEVIN

| Dimension | Josh Answers | Meet Kevin | My Decision |
|---|---|---|---|
| **Primary lens** | Catalyst-driven trading | Pricing power + fundamentals | **KEEP BOTH** — apply in sequence |
| **Time horizon** | Short-term momentum + long-term holds | Long-term capital appreciation | **KEEP BOTH** — depends on position tier |
| **Options strategy** | ITM calls/puts, Ghetto Spread | Sells puts occasionally, buys calls | **Josh wins** — Ghetto Spread is more defined |
| **OTC / Penny stocks** | Core content, rules-based | Doesn't touch | **Remove from this watchlist** — not monitored well remotely |
| **Stop losses** | Explicit; core rule | Rarely mentioned | **Josh wins** — no stop = no discipline |
| **Macro awareness** | Moderate — catalyst-focused | Strong — Fed/rate/inflation aware | **Kevin wins** — add macro overlay |
| **Community ethos** | Brotherhood / shared learning | Individual brand / course-driven | **Josh wins** — accountability matters |
| **Speculative names** | OTC with catalyst rules | Crypto mining, early-stage tech | **Both risky** — apply catalyst filter before touching either |
| **Real estate pillar** | Yes — major wealth pillar | Yes — Kevin's foundation | **Both agree** — real estate is real |
| **Concentration** | Budget discipline, sizing rules | High concentration, 50%+ in 3 stocks | **Josh wins** — Kevin's concentration hurt him |
| **Track record (picks)** | Documented buys: ORCL, INTC, SG, TAP | 33% accuracy tracked | **Mixed** — use frameworks not blind picks |
| **Pricing power lens** | Not formally used | Core filter | **Kevin wins** — run all picks through this filter |

---

### WHAT I'M THROWING OUT

| Item | Source | Why Discarded |
|---|---|---|
| Kevin's concentrated single-stock bets | Kevin | PP ETF failed partly from 50% in 3 names |
| Kevin's speculative micro-caps (HUT, Shift) | Kevin | No catalyst framework; pure narrative plays |
| Kevin's blind pick-following | Kevin | 33% accuracy; use his *framework*, not his *calls* |
| OTC/penny stock plays | Josh | Valid in live stream format; not monitorable in this dashboard |
| SG's AI/Infinite Kitchen moat | Both | Sweetgreen sold the Infinite Kitchen tech to Wonder (Nov 2025, $100M) — the tech moat is gone |
| Chasing INTC at current levels | Both | RSI 80+, up 5× from lows; the easy money is gone; Kevin prefers AMD/NVDA for AI infrastructure |

---

### WHAT I'M KEEPING AND COMBINING

| Insight | Origin | How It Changes the System |
|---|---|---|
| Catalyst before entry | Josh | Non-negotiable; still the trigger mechanism |
| Pricing power as screening filter | Kevin | Added BEFORE watchlist entry — no catalyst play without a pricing power story |
| Macro overlay | Kevin | Added rate/Fed awareness to long-term holds; rate-cut beneficiaries get higher conviction |
| Ghetto Spread on strong moves | Josh | Retained as the primary risk-management options tool |
| Stop levels on every position | Josh | Non-negotiable; Kevin has no equivalent |
| Long-term portfolio foundation | Both | Both agree — active trading sits on top of quality long holds |
| AI disruption as a theme | Kevin | Applied pricing power filter to AI names (LMND, AMD, ORCL all pass; INTC barely passes at current RSI) |
| Budget/position sizing | Josh | Non-negotiable; Kevin's failure was partly position sizing |
| Community accountability | Josh | The fraternity mindset: learn from collective mistakes |

---

### THE EVOLVED PHILOSOPHY

**Old (Josh-only):**
> Catalyst → Entry → Ghetto Spread → Net P&L

**Evolved (Josh + Kevin):**
> Pricing Power Screen → Macro Context Check → Catalyst Trigger → Entry with Stop → Ghetto Spread for risk management → Net P&L

**In plain English:**
Before touching any stock, ask three questions in order:
1. **(Kevin) Does this company have pricing power — can it raise prices without losing customers?** If no, it's not a long-term hold.
2. **(Kevin) Is the macro environment working with or against this thesis?** Rate-sensitive growth needs rate cuts; value needs patience; momentum needs volume.
3. **(Josh) What is the specific catalyst that triggers the entry?** News, earnings beat, product launch, institutional upgrade, government contract. No catalyst = no trade.

If all three pass → Enter with defined stop (Josh) → Use Ghetto Spread on big moves (Josh).

---

### ADDITIONAL REPUTABLE REFERENCE SOURCES ADDED

| Source | What It Adds |
|---|---|
| **Wedbush Equity Research** | Institutional price targets (ORCL $275, etc.) |
| **Morgan Stanley 2026 Outlook** | Macro risk assessment, recession probability framework |
| **J.P. Morgan Global Research** | Fed/rate cycle tracking |
| **Motley Fool / Seeking Alpha** | Secondary fundamental research, earnings call transcripts |
| **CNBC Markets** | Real-time catalyst news, analyst upgrades/downgrades |
| **TrueAlpha Data** | Creator pick accuracy tracking (used to calibrate Kevin's 33% rate) |

---

### WATCHLIST EVOLUTION — Before & After

| Ticker | Before (Josh-only) | After (Josh + Kevin) | Decision |
|---|---|---|---|
| ORCL | Long-Term Hold | **ELEVATED** — passes both frameworks | ✅ Keep, higher conviction |
| AMD | Momentum/Swing | **PRIMARY PICK** — best convergence of both | ✅ Keep, elevated to top pick |
| TAP | Dividend Hold | **REDUCED CONVICTION** — fails Kevin's pricing power filter | ⚠️ Keep, smaller size, Josh-only hold |
| SG | Swing/Growth | **THESIS WEAKENED** — sold Infinite Kitchen tech to Wonder Nov 2025 | ⚠️ Keep with shorter horizon, tighter stop |
| INTC | Watch Only | **ARCHIVED** — RSI 80+; Kevin prefers AMD/NVDA; Josh says don't chase | ❌ Remove from active, move to archive |
| LMND | Not on list | **NEW ADD** — Kevin's explicit buy; AI-native pricing power; profitability catalyst Q4 2026 | ✅ Add as AI Disruption play |

---

---

## WHO IS JOSH ANSWERS

- Real identity anonymous — has never shown his face publicly (safety + privacy)
- Born and raised in San Diego, CA; degree in Political Science and Philosophy, University of Arizona
- Trading since age 14; 15+ years of real-world experience across stocks, options, and real estate
- Streaming live on YouTube since ~2015
- Channels: **The Trading Fraternity** (~300K subs, educational) and **StockMarketLive** (~200K+ subs, daily live)
- Paid product: Stream alerts — $75/month
- Teaches 200+ people directly; countless more through free content

---

## THE THREE PILLARS OF WEALTH

1. **Stocks & Options Trading** — primary content, daily focus
2. **Real Estate** — 15+ years experience; secondary wealth-building arm
3. **Entrepreneurship** — financial literacy, mindset, income diversification

> *"My passion is spreading financial literacy and helping people harness their potential when it comes to making money and becoming a true entrepreneur."* — Josh Answers [direct]

---

## THE COMMUNITY ETHOS

- **"Band of brothers and sisters"** from around the country *[direct]*
- Mission: navigate the financial markets together, **learn from each other's mistakes**, and help each other grow along the way
- "In addition to fostering more profitable traders in both Bear and Bull markets, The Trading Fraternity sets the stage for making friendships that last a lifetime." *[direct]*
- Community-first: one person's mistake is everyone's lesson
- No one is left behind — newcomers are expected to paper trade first, not blow up and quit

---

## CORE TRADING RULES (The Foundational Framework)

### Rule 1 — There Are Rules. Learn Them.
> *"Whether trading NASDAQ, Blue-chip, or Penny stocks, OTC Pink Sheets — there are rules, and you better learn them."* *[direct]*

Different asset classes have completely different playbooks. You don't trade an OTC Pink Sheet the same way you trade Apple. Full stop.

### Rule 2 — This Is NOT a Get-Rich-Quick Scheme
- Repeated constantly, especially to new members
- Building real wealth takes time, discipline, consistency
- Anyone promising overnight riches is selling you something dangerous
- The goal is NET P&L over months and years — not one home run

### Rule 3 — Paper Trade First
- **Before real money hits the market, you simulate.** Period.
- Understand mechanics, practice entries and exits, build confidence — then scale up
- Losing fake money teaches you more than losing real money before you're ready

### Rule 4 — Stick to a Strict, Consistent Budget
- Every trade comes from a defined allocation — never blow past your budget
- Consistency in position sizing is how you survive long enough to win
- The budget protects your psychology as much as your account

### Rule 5 — Build a Long-Term Portfolio First
- Active trading is built on top of a foundation of quality long-term holds
- Blue chips and large-cap NASDAQ names anchor the portfolio
- Don't swing-trade everything — some positions are for holding, not flipping

### Rule 6 — Net P&L Is the Scoreboard
- One massive win doesn't make you profitable if 10 losses cancel it out
- Track everything. Know your real numbers.
- "One good trade doesn't make you a good trader."

### Rule 7 — Know the Difference Between a Catalyst and a Pump
- **Catalyst-driven move**: press release, earnings, FDA approval, macro news — legitimate reason for the move
- **Pump**: manufactured hype, social media run-up with no substance — high-risk, not a trade, a gamble
- If you can't identify the catalyst, you don't have a trade — you have a prayer

### Rule 8 — Know Your OTC/Pink Sheet Rules Cold
- OTC Pink Sheets are a different animal — **higher risk, more volatile, more manipulation**
- Rules: smaller position size, tighter stop discipline, only trade with a clear catalyst, know your exit before entry
- These aren't long-term holds — get in, get out, don't get greedy

### Rule 9 — Losses Are Lessons
- Every loss has something to teach — track what went wrong and why
- The community learns collectively; share the mistake so someone else doesn't repeat it
- Cutting a loss at 20% is discipline. Riding it to -80% is ego.

### Rule 10 — Not Trading IS a Valid Trade
- On slow days, in uncertain markets, the best position is sometimes no position
- Waiting for the right setup is a skill — don't force trades out of boredom or FOMO

---

## OPTIONS FRAMEWORK (Josh's Approach)

### ITM vs. OTM
- **In The Money (ITM)**: higher premium, lower risk, behaves more like owning shares
  - *Josh's general preference for directional plays* — more certainty, more intrinsic value
- **Out of The Money (OTM)**: cheaper, higher risk — only worth it if you're expecting a large, fast move
  - "OTM options can be profitable if you expect a big move, but they're riskier and less expensive." *[direct]*

### The Ghetto Spread — Josh's Signature Strategy
**How it works:**
1. Buy a call or put option on a strong catalyst play
2. Let it increase in value (the move is working in your favor)
3. Sell a further OTM call or put for **more than what you originally paid** for the first contract
4. Result: Your original position is now **free** — zero cost basis, with upside still open if the stock keeps moving
5. You've locked in profit without using a day trade

> *"The idea of a Ghetto Spread is securing profits on a Call or Put option without using a day trade, and giving yourself a chance to make a little more if the stock moves in your favor."* *[direct]*

**Famous trade:** Used the Ghetto Spread to turn ~$500 into $40,000–$50,000 on a single play.  
**Notable stat:** On one live stream, Josh executed a put option that returned 1,000% gains in 15 minutes.

### Options Basics Josh Drills
- Options value is tied to whether the asset is "in the money" or "out of the money"
- Intrinsic value vs. time value — understand what you're paying for
- Premium erodes — time decay (theta) works against you on OTM holds
- Only buy options on liquid tickers — wide spreads kill your edge on illiquid names

---

## MARKET CATEGORIES & APPROACH

| Category | Examples | Framework |
|---|---|---|
| NASDAQ Blue Chips | AAPL, MSFT, NVDA, AMZN | Long-term portfolio holds; fundamentals matter |
| Mid-Cap Growth | Momentum names, sector leaders | Swing/momentum plays; catalyst-driven |
| Penny Stocks / OTC | Pink Sheets, sub-$5 names | Strict rules; catalyst only; tight stops; small size |
| Options | Calls/puts on liquid NASDAQ tickers | ITM preferred; Ghetto Spread for risk mgmt |

---

## DAILY WORKFLOW JOSH MODELS

### Pre-Market (Before 9:30 AM ET)
1. Scan for overnight gappers and pre-market movers
2. Identify the **catalyst** — news, PR, earnings, macro event, tweet
3. Filter: Is this a real catalyst or a pump? 
4. Set watchlist: price levels, entry zones, stop levels
5. Know your budget allocation before the bell rings

### Market Open (9:30 AM – 11:00 AM ET)
- Highest volatility window — execute with discipline
- Live commentary: narrate the trade, explain the thesis in real time
- Don't chase — if you missed the entry, let it go
- Respect pre-defined stops. No moving the goalposts.

### Mid-Day (11:00 AM – 2:00 PM ET)
- Hold/cut decisions based on price action and volume
- Low volume = be cautious; momentum can stall or reverse
- Take partial profits on big moves; let a runner run with a free position (Ghetto Spread logic)

### Post-Market (After 4:00 PM ET)
- Recap: what worked, what didn't, why
- Identify next-day setups
- Community debrief — what can we learn collectively?

---

## COMMUNICATION STYLE PROFILE

- **Conversational and direct** — no jargon dumping, no academic language
- Calls out bad habits **bluntly but motivationally** — "you better learn these rules" not "I suggest you consider"
- Mixes market commentary with life philosophy naturally — wealth mindset, discipline, brotherhood
- **Repeats key rules constantly** — especially for newer members who need to hear it 50 times before it sticks
- Always disclaims: "this is educational, not advice"
- Keeps energy up even on slow market days — slow days are teaching days
- Sense of humor, personality, real-talk about L.A. life, keeps it human
- Anonymous by design — the focus stays on the trade, not the personality

---

## TOP KNOWN/CONFIRMED YOUTUBE VIDEOS
*(Sourced from public search results and YouTube links — verified titles)*

### Evergreen / High-View Education Content

| # | Video Title | Theme |
|---|---|---|
| 1 | How I Turned $500 Into $40,000 In ONE TRADE – The Ghetto Spread On Robinhood | Signature options strategy |
| 2 | 1000% Profits in 2 DAYS using this Options Strategy – GHETTO SPREAD Options Trading Made Easy | Ghetto Spread tutorial |
| 3 | HOW TO TRADE THE GHETTO SPREAD \| EP. 7 | Ghetto Spread deep dive |
| 4 | The Trading Fraternity's Ghetto Spread Explained!! (Best Options Trading Strategy) 🔥 | Ghetto Spread explainer |
| 5 | THE TRADING FRATERNITY GHETTO SPREAD EXPLAINED | Community version / breakdown |
| 6 | How to do Ghetto Spread Options Trading by the Fraternity | Step-by-step tutorial |
| 7 | Trading Fraternity's Ghetto Spread – A MUST WATCH VIDEO | Highlight / intro video |
| 8 | How I Turned $600 Into $7,000 With OPTIONS – Ghetto Standard Deviation | Variation on the spread |
| 9 | How To Trade Options: In The Money (ITM) VS. Out The Money (OTM) | Options 101 |
| 10 | Meeting Option Traders In LA That Follow Me – EXPOSING Josh Answers & The Trading Fraternity | Community / culture |
| 11 | The END Of The WATCHLIST (Members-only migration announcement, April 2025) | Watchlist format update |
| 12 | Watchlist 1/23/20 (Podcast/Spotify) | Early watchlist format |

### 2025–2026 YTD Uploads (Confirmed Titles, Chronological)

| Date | Video Title | Ticker(s) | Theme |
|---|---|---|---|
| Oct 5, 2025 | I Bought SG & CHIPOTLE STOCK – Rate Cut Will SAVE or SINK The Market | SG, CMG | Consumer discretionary + macro |
| Dec 4, 2025 | I Bought TAP Stock For THE LONG TERM | TAP | Value / dividend long-term hold |
| Jan 20, 2026 | The Banking Crisis IS HAPPENING RIGHT NOW!! | — | Macro / sector warning |
| Feb 2, 2026 | I'm BUYING This BORING Stock While The Economy RUNS HOT – 3 Stocks for 2026 | TBD | Value buys in hot economy |
| Feb 9, 2026 | Why Everyone's Wrong About The Dollar Collapsing | — | Macro / contrarian dollar thesis |
| Feb 16, 2026 | The Macro Indicator That Will Show a MARKET CRASH or MARKET BOOM | — | Key macro signal education |
| Feb 23, 2026 | The AI Future Just Revealed the Market's Biggest Lie | — | AI valuation reality check |
| Mar 5, 2026 | STOP Trading Until You Watch This: Midterm Money Glitch | — | Market cycle / timing education |
| Mar 7, 2026 | I Bought ORCL Stock Down 50% (Benjamin Graham's Secret Model) | ORCL | Value investing at capitulation |

> **Note:** Full top-20 view-count ranking requires direct YouTube channel access. The above are the highest-visibility confirmed titles from public search indexing. Josh's daily live streams (StockMarketLive) accumulate the bulk of total watch time but aren't ranked individually by views in the same way. **"Call up"** — when the user says this phrase, JoshAnswers triggers a fresh research pull on the most recent Trading Fraternity videos and live streams.

---

## JOSH'S CONFIRMED STOCK POSITIONS (From Public Video Sourcing)

| Ticker | Company | When Josh Bought | Entry Context | Current Price (May 24, 2026) | Status |
|---|---|---|---|---|---|
| SG | Sweetgreen | Oct 2025 | Rate-cut play; growth restaurant thesis | ~$9.60 | Up 38.9% YTD; still 40% below Jul 2025 high of $16.26 |
| CMG | Chipotle | Oct 2025 | Rate-cut play; premium fast casual | — | Confirm current price |
| TAP | Molson Coors | Dec 2025 | Long-term value/dividend hold | ~$44.04 | −29% over past year; 4.58% yield; ex-div May 29 |
| ORCL | Oracle | Mar 2026 | Benjamin Graham value model; bought −50% from ATH | ~$190.88 | Recovering from $134 low; Wedbush PT $275 |
| INTC | Intel | Ongoing 2026 | Called daily on Threads; manufacturing turnaround | ~$119 | +400%+ YTD; RSI ~80 (extended) |

---

## RECURRING PHRASES & MANTRAS

| Phrase | Context |
|---|---|
| *"There are rules — you better learn them."* | Said about every asset class, especially OTC |
| *"Band of brothers and sisters"* | Community identity |
| *"This is NOT financial advice — this is education"* | Every live stream disclaimer |
| *"Paper trade first"* | Always said to newcomers |
| *"Stick to your budget"* | Position sizing discipline |
| *"Net P&L"* | The real scoreboard, not cherry-picked wins |
| *"Know your catalyst"* | Before any trade is placed |
| *"Don't chase it"* | If you missed the entry |
| *"Cut your losses"* | Discipline over ego |
| *"The fraternity learns together"* | After community mistakes |
| *"Fastest news in the world"* | How his live stream is described |
| *"Real estate, stocks, entrepreneurship — three pillars"* | Wealth framework |

---

---

## JOSHANSWERS WATCHLIST v3 — May 25, 2026
> *Built through the evolved framework: Pricing Power Screen → Macro Check → Catalyst Trigger → Entry with Stop.*  
> *Josh Answers (catalyst, discipline, stops, options) + Meet Kevin (pricing power, fundamentals, AI disruption, macro) + Institutional research.*  
> ***7 active names. 1 archived. Grown from 5 → 7: added MSFT (AI backbone, pricing power maximum) and NVDA (AI infrastructure, Blackwell ramp catalyst).***  
> *Not financial advice — educational analysis only.*

---

### NEW ADDITIONS — May 25, 2026

#### WHY 2 STOCKS? WHY THESE 2?

The evolved framework demands every new watchlist entry passes three gates in sequence:
1. **(Kevin) Does this company have pricing power?** Can it raise prices without losing customers?
2. **(Kevin) Is macro working for or against this thesis?** Rate environment, sector tailwinds.
3. **(Josh) What is the specific catalyst that triggers the entry?** No catalyst = no trade.

After running the full opportunity set through these three gates, **MSFT and NVDA emerge as the two strongest additions.** Both score maximum on pricing power (no enterprise substitute for Azure; no AI chip substitute for Blackwell), both have clear near-term catalysts, and both sit within tradeable range — not extended like INTC at RSI 80+.

---

### NEW PICK A: MSFT — Microsoft Corporation
**Tier: 1 (AI Backbone — Long-Term Hold) | MAXIMUM CONVICTION — Both frameworks ✅**
*Pricing Power Screen: MAXIMUM PASS — enterprise is locked into Microsoft ecosystem: Office 365, Teams, Azure, GitHub Copilot. Switching cost is catastrophic for any organization. Customers cannot leave.*
*Macro Check: PASS — Azure cloud = secular growth; rate-neutral; enterprise spending is sticky even in slowdowns*
*Catalyst: PASS — Azure growing +40% YoY; 20M Copilot enterprise seats generating $30/seat/month in incremental revenue; Q4 FY2026 earnings in July; Wedbush PT $625, Morgan Stanley PT $650*

| Metric | Data |
|---|---|
| Entry Zone | $420–$435 (any pullback to support) |
| Price Target | $625 (Wedbush) / $650 (Morgan Stanley) |
| Stop | $395 (below multi-quarter support) |
| Azure Revenue Growth | +40% YoY (accelerating) |
| Copilot Enterprise Seats | 20M at $30/seat/mo = $600M/mo incremental |
| Framework Score | Both frameworks: maximum |

**Why MSFT earns Tier 1 over even ORCL:**
Microsoft wins the Kevin pricing power test at the highest level — no other company charges enterprises more and has less churn. Every Office 365 subscription auto-renews. Every Azure customer gets deeper into the ecosystem with each passing quarter. When Microsoft raises Copilot prices (and they will), customers pay.

Under Josh's framework: the catalyst is *compounding* rather than a single event — each quarterly earnings is a catalyst window. The July Q4 FY2026 earnings release is the next hard date. Wedbush's $625 target implies ~45% upside from entry.

**Ghetto Spread setup:** Buy ITM call ($420 strike) ahead of earnings → sell further OTM call for more than original debit if the stock spikes on results. Classic Josh execution on a predictable catalyst event.

**Entry:** $420–$435 on any pull. $395 is non-negotiable stop — breaks long-term support.
**Catalyst to watch:** Q4 FY2026 earnings (July 2026), Copilot seat count update, Azure rev growth acceleration/deceleration.

---

### NEW PICK B: NVDA — NVIDIA Corporation
**Tier: 2 (AI Infrastructure — Momentum/Swing) | HIGH CONVICTION — Both frameworks ✅**
*Pricing Power Screen: PASS — NVIDIA has a 70-80% market share in AI training chips. Blackwell architecture is 4× the performance of Hopper. Hyperscalers cannot use AMD, Intel, or anyone else at this compute density. Pricing power is the thesis.*
*Macro Check: PASS — AI infrastructure spending is the one area where capex is accelerating regardless of rate environment. China market reopening (H200 cleared) adds a $10B+ revenue layer back to the equation.*
*Catalyst: PASS — Blackwell GB300 (successor to H100) ramp in Q3 2026; China H200 export license cleared by Commerce Dept; Q2 FY2027 earnings guidance from Jensen; $500B revenue visibility from hyperscaler commitments*

| Metric | Data |
|---|---|
| Q1 FY2026 Revenue | $81.6B (+85% YoY) |
| Entry Zone | $130–$140 (post-split; pullback to 50-day MA) |
| Price Target | $185 (near-term) |
| Stop | $112 (below 52-week breakout level) |
| Blackwell Ramp | GB300 shipping Q3 2026 |
| China Catalyst | H200 export cleared — reopens $10B+ market |
| Jensen Catalyst | Returned from Beijing May 2026 — deal momentum |

**Why NVDA now after being flat YTD (+15%):**
The YTD flatness is the opportunity — NVDA underperformed the AI trade in 2026 while AMD ran +66%. The reason: China export restrictions kept ~$10B+ of revenue off the table. That restriction just changed. Jensen Huang's Beijing visit in May 2026 signals the relationship is normalizing. The Blackwell GB300 ramp in Q3 gives another hardware catalyst.

Josh's framework: unusual call premium on NVDA options (noted by CNBC May 20, 2026) is the kind of institutional tell Josh watches — the smart money was positioning before the move. The Ghetto Spread setup here is exactly what Josh has described: buy ITM calls ahead of a confirmed catalyst (Blackwell + China), let the move happen, sell further OTM calls for more than original debit.

**Entry:** $130–$140 on any pullback to the 50-day MA zone.
**Stop:** $112 — below the pre-China catalyst breakout level; thesis is broken.
**Target:** $185 near-term; $225+ if China revenue resumes and Blackwell demand exceeds guidance.
**Catalyst to watch:** Q2 FY2027 earnings guidance, any Blackwell volume announcement, China deal formalization.

---

### WATCHLIST SUMMARY TABLE — v3 (7 Active Picks)

| # | Ticker | Company | Tier | Entry Zone | Target | Stop | Framework | Primary Catalyst |
|---|---|---|---|---|---|---|---|---|
| 1 | MSFT | Microsoft | 1 — AI Backbone 🔼 | $420–$435 | $625 | $395 | **Both (max)** | Azure +40%; Copilot monetization; Q4 FY26 earnings |
| 2 | NVDA | NVIDIA | 2 — AI Infrastructure 🔼 | $130–$140 | $185 | $112 | **Both** | Blackwell GB300 ramp; China H200 cleared; $500B visibility |
| 3 | ORCL | Oracle | 1 — Long-Term 🔼 | $175–$185 | $275 | $165 | **Both** | AI cloud RPO $523B; Wedbush PT |
| 4 | AMD | Advanced Micro Devices | 2 — Primary Pick 🔼 | $390–$400 | $469 | $370 | **Both** | MI400 Q3 launch; $60B hyperscaler commits |
| 5 | LMND | Lemonade | 2 — AI Disruption 🆕 | $60–$65 | $75–$90 | $52 | **Kevin-led** | Q4 EBITDA profitability; Tesla partnership |
| 6 | TAP | Molson Coors | 1 — Dividend ⚠️ | $40–$44 | $49 | $38 | **Josh only** | 4.58% yield; ex-div May 29 |
| 7 | SG | Sweetgreen | 2 — Short Swing ⚠️ | $9.00–$9.50 | $13 | $7.50 | **Josh only** | Q2 wrap sales data |
| — | INTC | Intel | ARCHIVED ❌ | $95–$105 re-entry | $132+ | — | Was: Both | Wait for RSI cooldown |

> **Total active picks: 7** (was 5, added MSFT + NVDA)
> **Conviction ranking:** MSFT > NVDA > ORCL > AMD > LMND > TAP > SG

---

## JOSHANSWERS WATCHLIST v2 — May 24, 2026 (SUPERSEDED BY v3)
> *Historical record — v2 ran from May 24 to May 25, 2026. Superseded by v3 above.*  
> *5 active names. 1 archived.*

---

### WATCHLIST LEGEND
- **Tier 1 — Long-Term Hold** (portfolio foundation, Josh's pillar)
- **Tier 2 — Swing/Momentum** (catalyst-driven, defined entry/exit)
- **Tier 3 — Watch Only** (extended/overbought — wait for pullback before touching)

---

### 1. ORCL — Oracle Corporation
**Tier: 1 (Long-Term Hold) | ELEVATED — Both frameworks converge ✅**
*Pricing Power Screen: PASS — enterprise database lock-in; customers don't leave Oracle*
*Macro Check: PASS — AI capex supercycle is a secular trend; rate-neutral*
*Catalyst: PASS — $523B RPO, AI cloud contracts, Wedbush PT $275, Q3 beat*

| Metric | Data |
|---|---|
| Current Price | ~$190.88 (May 24, 2026) |
| 52-Week Range | $134.57 – $345.72 |
| Josh's Entry Zone | ~$130–$152 (March 2026, bought −50% from ATH) |
| Analyst Price Target | $275 (Wedbush, raised May 13, 2026) |
| Upside to PT | ~44% from current |
| AI Cloud Backlog (RPO) | $523 billion |

**Why it's on the watchlist:**
Josh applied Benjamin Graham's value model — the stock got obliterated (−62% from ATH) due to fear around $50B CapEx and $100B+ debt, not because the business broke. The underlying fundamentals were *strengthening*: OpenAI, Meta, and NVIDIA all signed massive OCI cloud contracts. RPO hit $523B — that's forward revenue visibility. Q3 2026 was the best growth quarter in years.

This is Josh's framework in textbook form: **identify the catalyst for the sell-off, verify the business hasn't fundamentally broken, buy at maximum fear.** The stock is in recovery mode at $190, still 45% below its ATH of $345. Wedbush's $275 target gives a clear intermediate thesis.

**Entry:** Already in play at these levels (Josh entered lower). New positions look for $175–$185 support zone on any pullback.  
**Stop zone:** Below $165 would suggest the recovery thesis is failing.  
**Catalyst to watch:** Q4 2026 earnings, AI cloud contract announcements, debt reduction progress.

---

### 2. AMD — Advanced Micro Devices
**Tier: 2 (Primary Momentum Pick) | ELEVATED — Best convergence of both frameworks ✅**
*Pricing Power Screen: PASS — hyperscalers pay AMD premium; no commodity pricing; MI400 has no direct substitute*
*Macro Check: PASS — AI infrastructure spending is accelerating regardless of rate environment*
*Catalyst: PASS — MI400/Helios racks shipping Q3 2026; $60B hyperscaler commitments locked*

| Metric | Data |
|---|---|
| Current Price | ~$421 (May 2026) |
| 52-Week Range | $107.67 – $469.22 |
| 2026 YTD Gain | +66% |
| Q1 2026 Revenue | $10.3B (+38% YoY) |
| Data Center Revenue | $5.8B Q1 (+57% YoY) |
| Next Hard Catalyst | MI400 series / Helios racks shipping Q3 2026 |
| Hyperscaler Commitments | OpenAI 6GW + Meta 6GW (~$60B+ multi-year) |

**Why it's on the watchlist:**
AMD is winning the AI infrastructure rotation. While NVDA went relatively flat in 2026 (+15%), AMD ran +66% YTD on real, verified revenue growth. The MI400 MI455X (40 PFLOPS, 432 GB HBM4) shipping in Q3 is a product catalyst — not hype, shipping hardware. Hyperscalers have committed to the platform with multi-year, gigawatt-scale deployments.

Under Josh's framework: **this has a real catalyst (MI400 launch), fundamental backing (record revenues), and momentum.** The stock has room to run toward the 52-week high of $469.

**Entry:** Ideal entry is a pullback to the $390–$400 zone, which held as support after the Q1 earnings spike. Current $421 is acceptable for a swing position with a clear stop.  
**Stop zone:** Below $370 breaks the post-earnings support structure.  
**Catalyst to watch:** MI400 volume shipment confirmation, Q2 2026 data center revenue, any hyperscaler capex revisions.

---

### 3. TAP — Molson Coors Beverage Company
**Tier: 1 (Dividend Hold — REDUCED CONVICTION) | Josh-only hold ⚠️**
*Pricing Power Screen: FAIL — declining brand relevance; consumers shifting to craft/premium/alternative beverages; TAP is a price-taker, not a price-setter*
*Macro Check: NEUTRAL — benefits from consumer spending stability; hurt by premiumization trend*
*Catalyst: WEAK — dividend + buybacks, but no growth catalyst. The ex-dividend May 29 is a near-term income event, not a growth story.*
*Kevin verdict: Would not hold. No innovation. No moat. Josh verdict: Bought Dec 2025 for income.*
*My decision: Keep as small, Josh-only income position. NOT a conviction buy for new money.*

| Metric | Data |
|---|---|
| Current Price | ~$44.04 (May 2026) |
| 52-Week Performance | −29% |
| Annual Dividend | $1.92/share |
| Dividend Yield | 4.58% |
| Next Ex-Dividend Date | **May 29, 2026 (THIS WEEK)** |
| Dividend Growth (3yr avg) | +20.38%/year |
| Analyst Avg Rating | Hold; PT ~$48.94 (+10% upside) |

**Why it's on the watchlist:**
Josh bought TAP in December 2025 as a long-term portfolio hold — this is pure Josh framework: boring, beaten-down, dividend-paying business while everyone else chases AI. The stock is down 29% over the past year, trading at $44. It's not glamorous — that's the point.

The 4.58% dividend yield is real money while you wait. The company is executing a premiumization strategy (premium brands, share buybacks) and the *next ex-dividend date is May 29* — that's this week. If you want to capture the next dividend, the window is closing.

**Entry:** Already at value levels near $44. Any dip toward $40 is a deeper value entry.  
**Stop zone:** Below $38 (dividend cut risk would likely accompany a move this low).  
**Catalyst to watch:** Q2 2026 earnings showing premiumization traction, consumer spending recovery, any dividend increase announcement.

---

### 4. SG — Sweetgreen, Inc.
**Tier: 2 (Swing — THESIS REVISED, SHORTER HORIZON) | Josh catalyst play, AI moat removed ⚠️**
*Pricing Power Screen: PARTIAL FAIL — Sweetgreen sold its Infinite Kitchen robotics tech (Spyce) to Wonder in November 2025 for $100M cash + $86.4M stock. The operational AI edge remains (they buy it back at cost+5%), but they no longer OWN the technology moat. Kevin would exit on this news.*
*Macro Check: NEUTRAL — consumer discretionary; rate cuts help, but fast-casual is highly competitive*
*Catalyst: ALIVE BUT NARROWER — wrap menu launch under $15 is a real short-term catalyst; JPMorgan Overweight upgrade, PT $13; Q2 2026 same-store sales data is the binary event*
*My decision: Shorten the time horizon. This is now a pure Josh catalyst swing — wrap sales data in Q2. Tighter stop. Exit if Q2 same-store sales disappoint. Not a long-term hold.*

| Metric | Data |
|---|---|
| Current Price | ~$9.60 (May 23, 2026) |
| 52-Week High | $16.26 (July 2025) |
| YTD 2026 Performance | +38.9% |
| Recent Low | ~$6.51–$6.89 (May 2026) |
| Move Off Recent Low | ~+45% |
| JPMorgan PT | $13 (upgraded to Overweight) |
| Oppenheimer PT | $10 (reiterated Outperform) |

**Why it's on the watchlist:**
Josh bought SG in October 2025 as a rate-cut play — the thesis: lower rates relieve pressure on growth restaurant concepts burning cash to scale. That macro thesis is still in play. The stock got hammered to ~$6.50 in early May 2026 after a soft Q1, then ripped +45% in two weeks on the national launch of high-protein wraps priced under $15.

This is the kind of **catalyst-driven move Josh watches on growth names**: a product pivot that changes the consumer conversation. Wraps = more accessible price point = broader TAM. JPMorgan's upgrade to Overweight validates the momentum.

At $9.60, it's still 41% below the July 2025 high of $16.26. The JPMorgan $13 target suggests another 35% from here if execution holds.

**This is a higher-risk position — smaller size applies under Josh's budget rules.**  
**Entry:** $9.00–$9.50 zone on any pullback from the recent spike.  
**Stop zone:** Below $7.50 breaks the recovery thesis.  
**Catalyst to watch:** Q2 2026 same-store sales (first full wrap quarter), margins improving, any profitability milestone.

---

### 5. LMND — Lemonade, Inc. *(NEW — replacing INTC as active pick)*
**Tier: 2 (AI Disruption / Swing) | Kevin's explicit buy; passes evolved framework ✅**
*Pricing Power Screen: STRONG PASS — AI-native insurance with 6% LAE ratio vs. industry 8-10%; lower costs than incumbents = structural pricing advantage; 90% of customers are first-time buyers (blue ocean TAM)*
*Macro Check: PASS — rate cuts help growth stocks; insurance is relatively rate-neutral for premiums*
*Catalyst: PASS — Q1 2026 revenue +71% YoY; Tesla partnership (autonomous car insurance at 50% discount); path to EBITDA profitability Q4 2026; CEO says company is "on a tear"; 4 straight weeks of stock gains*

| Metric | Data |
|---|---|
| Current Price | ~$67 (May 2026) |
| 52-Week Range | $25.85 – $99.90 |
| 2025 Performance | +94% |
| 2026 YTD | −24% (from high; recently recovering) |
| Q1 2026 Revenue Growth | +71% YoY |
| In-Force Premium | $1.33B (+32% YoY, 10th consecutive quarter of acceleration) |
| LAE Ratio | 6% (vs industry 8–10% — structural cost advantage) |
| Customers | ~2.9M (growing; headcount shrinking = AI leverage) |
| EBITDA Profitability | Guided Q4 2026 |
| Analyst Consensus PT | $68–70 avg; Morgan Stanley Buy PT $75; range $33–$103 |
| Tesla Partnership | Launched autonomous car insurance; ~50% discount for FSD miles |
| Kevin's position | Publicly stated buyer |

**Why LMND earns the spot over INTC:**
- INTC is up 5× already — the pricing power thesis there is real but the entry is RSI-overextended
- LMND is down 24% YTD from its high, recovering with strong fundamentals — better risk/reward at current levels
- LMND passes the Kevin pricing power filter AND the Josh catalyst trigger
- The profitability milestone (Q4 2026 EBITDA positive) is the kind of binary catalyst Josh looks for

**Entry:** $60–$65 on any pullback (current ~$67 is acceptable on dips)
**Stop zone:** Below $52 — breaks the 2026 recovery structure
**Target:** $75 (Morgan Stanley) → $90+ if EBITDA profitability hits on time
**Catalyst to watch:** Q2 2026 earnings (August), EBITDA guidance update, Tesla partnership expansion

---

### 6. INTC — Intel Corporation *(ARCHIVED — removed from active picks)*
**Tier: ARCHIVED | Was: Watch Only**

| Metric | Data |
|---|---|
| Price at Archive | ~$119 (May 24, 2026) |
| 52-Week Low | ~$19.98 (May 2025) |
| 52-Week High | $132.75 |
| YTD Performance | +400%+ |
| RSI (14) | ~80.5 — OVERBOUGHT |
| Market Cap | ~$503B |

**Why it's archived:**
Josh called Intel daily and the thesis was right — CHIPS Act, Apple foundry talks, Terafab. The 5× move happened. But RSI 80.5 is deep overbought. Kevin prefers AMD and NVDA for AI infrastructure. Both frameworks say the same thing: **the easy money is gone at these levels.** This is now a "watch on re-entry" candidate — not an active position.

**Re-entry trigger:** Pullback to $95–$105, RSI cooling to 50–55, volume stabilization.
**Archive date:** May 24, 2026. Will revisit if fundamentals shift (Apple contract signed, Terafab volume announced).

---

### WATCHLIST SUMMARY TABLE — v2 (Post-Kevin Evolution)

| Ticker | Company | Tier | Price (May 24) | Entry Zone | Target | Stop | Framework | Catalyst |
|---|---|---|---|---|---|---|---|---|
| ORCL | Oracle | 1 — Long-Term 🔼 | $190.88 | $175–$185 | $275 | $165 | **Both** | AI cloud RPO, Wedbush PT |
| AMD | Advanced Micro Devices | 2 — Primary Pick 🔼 | $421 | $390–$400 | $469 | $370 | **Both** | MI400 Q3 launch |
| LMND | Lemonade | 2 — AI Disruption 🆕 | ~$67 | $60–$65 | $75–$90 | $52 | **Kevin-led** | Q4 EBITDA, Tesla partnership |
| TAP | Molson Coors | 1 — Dividend ⚠️ | $44.04 | $40–$44 | $49 | $38 | **Josh only** | Ex-div May 29 🔔 |
| SG | Sweetgreen | 2 — Short Swing ⚠️ | $9.60 | $9.00–$9.50 | $13 | $7.50 | **Josh only** | Q2 wrap sales data |
| INTC | Intel | ARCHIVED ❌ | $119 | $95–$105 re-entry | $132+ | — | Was: Both | Wait for RSI cooldown |

> **"Call up"** — say this phrase any time and JoshAnswers will pull the latest Trading Fraternity and Meet Kevin videos, live stream recaps, and social activity to refresh the watchlist with new intelligence.

---

## YTD 2026 MARKET CONTEXT (Jan – May 24, 2026)

### Index Performance (as of May 15, 2026)
| Index | YTD Return |
|---|---|
| NASDAQ | +13.1% |
| S&P 500 | +8.7% |
| Dow Jones | +3.6% |

- S&P 500 posted its **longest 8-week winning streak since December 2023** through mid-May
- Market pivot: value and dividend-focused plays have outperformed pure tech/growth in 2026

### Sector Leaders YTD
- **Semiconductors (ex-NVDA):** Intel (+200%+), AMD (+267% 12-month), Micron — AI infrastructure shift
- **Specialty Chemicals:** Albemarle (ALB) +28% YTD (lithium/EV demand)
- **Energy Services:** Noble Corp (NE) +27%, Patterson-UTI (PTEN) +23%
- **AI Megacap shift:** NVDA relatively flat +15% vs. prior years; Intel, AMD, Micron taking the torch

### Notable 2026 Story Stocks (through May 24)

| Ticker | YTD Move | Price (May 24) | Catalyst |
|---|---|---|---|
| INTC | +400%+ | ~$119 | CHIPS Act, Terafab (SpaceX/xAI/Tesla), Apple foundry, govt 10% stake; RSI 80 — extended |
| AMD | +66% YTD (+114% last 12mo) | ~$421 | MI400 data center chips, record $5.8B Q1 DC revenue, $60B hyperscaler commitments |
| ORCL | Recovering from −62% crash | ~$190.88 | AI cloud RPO $523B; Wedbush PT $275; down from $345 ATH but recovering |
| NVDA | +15% YTD | — | Unusual call premium pre-earnings; positive skew; AI infrastructure pivot |
| SG | +38.9% YTD | ~$9.60 | Wrap launch catalyst; JPMorgan upgrade; recovering from $6.50 low |
| TAP | −29% last 12mo | ~$44 | Beaten-down value; 4.58% dividend; ex-div **May 29** |
| ALB | +28% YTD | — | Lithium demand for EV/energy storage |
| NINE | +2,951% YTD | — | Speculative micro-cap mover |
| MGRT | +1,094% YTD | — | Speculative micro-cap mover |

### Josh's Confirmed 2026 Video Plays (Verified Sourcing)

| Video Date | Ticker(s) | Context | Outcome (as of May 24) |
|---|---|---|---|
| Oct 5, 2025 | SG, CMG | Rate-cut play; growth restaurants | SG: +38.9% YTD from Jan; up ~45% from May low |
| Dec 4, 2025 | TAP | Long-term value/dividend | TAP flat to slightly down from entry; dividend payments ongoing |
| Mar 7, 2026 | ORCL | Benjamin Graham value buy at −50% from ATH | ORCL at ~$190 vs. ~$130-150 entry; up ~25-45% from Josh's entry |
| Ongoing 2026 | INTC | Called daily on Threads | INTC: up 5x from lows; now extended (RSI 80) |

---

## REST OF YEAR OUTLOOK (May 25, 2026 – December 2026)

> *In the spirit of how Josh frames market setups — not personalized advice.*

### Key Themes to Watch Through EOY 2026
1. **Intel (INTC) continuation play** — Government-backed manufacturing is a multi-year thesis; pullbacks are potential buy-the-dip setups under Josh's framework
2. **AMD data center dominance** — MI400 series shipping Q3 2026 is a near-term catalyst window
3. **AI infrastructure rotation** — Money has been rotating from software AI plays into hardware/chip infrastructure; this has legs
4. **Energy & commodities** — Offshore drilling stocks had big YTD runs; watch for continuation vs. mean reversion setups
5. **OTC/Penny catalyst plays** — The micro-cap space (NINE, MGRT-level movers) has been active in 2026; Josh's catalyst-first framework is exactly the right lens here
6. **Options opportunities** — NVDA's unusual call premium pre-earnings is the kind of setup Josh watches; if NVDA breaks out post-earnings, ghetto spread setups become available

### Risk Factors to Monitor
- Fed policy and rate direction (affects growth/tech multiples)
- Geopolitical developments (semiconductor supply chains)
- AI hype cycle fatigue vs. real revenue delivery
- Micro-cap pump risk elevated in hot OTC market — know your catalyst

### Josh's Framework Applied to H2 2026
- **Don't chase already-extended plays** (INTC +200%, AMD +267%) without clear re-entry catalyst
- **Watch for sector rotation setups** — what's lagging that has a catalyst to catch up?
- **Long-term portfolio principle**: Blue chips on any meaningful pullback (AAPL, MSFT, NVDA dips) are buying opportunities for the long-term leg
- **Budget discipline matters more** in a hot market — FOMO is highest when everything is ripping

---

## MOST RECENT KNOWN PLAYS / MENTIONS (Spring 2026)

| Ticker | Context | Price Context | Source |
|---|---|---|---|
| $INTC | Called daily by Josh on Threads; Intel manufacturing turnaround; now RSI 80+ — watch for pullback re-entry | ~$119; 5x from lows | Threads @thetradingfraternity |
| $ORCL | Josh bought in March 2026 using Benjamin Graham model; bought −50% from ATH | Entry ~$130-$152; now ~$190; Wedbush PT $275 | YouTube Mar 7, 2026 |
| $TAP | Josh's long-term portfolio hold from Dec 2025; 4.58% dividend; ex-div May 29 this week | ~$44; down from entry | YouTube Dec 4, 2025 |
| $SG | Josh's Oct 2025 rate-cut buy; wraps launch catalyst driving recovery; JPM upgraded | Entry Oct 2025; now ~$9.60; JPM PT $13 | YouTube Oct 5, 2025 |
| $CMG | Josh's Oct 2025 buy alongside SG; premium fast casual rate-cut thesis | Confirm current price | YouTube Oct 5, 2025 |
| $AMD | Data center record revenues; MI400 Q3 launch; +66% YTD momentum with fundamental backing | ~$421; 52w range $107-$469 | Public market data |
| $NVDA | Unusual call skew pre-earnings; ITM call ghetto spread opportunity if earnings break out | +15% YTD; extended but still in play | CNBC May 20, 2026 |

---

## SELF-IMPROVEMENT LOG (JoshAnswers Agent)

### Call Up Research — May 25, 2026

**Research sweep triggered by user "call up" command. Sources: YouTube public index, Yahoo Finance, TheStreet, CNBC, Motley Fool, StockTwits, StocksToTrade, public analyst reports.**

#### Most Recent Trading Fraternity / StockMarketLive Content
- Most recent publicly indexed video: *"I Bought ORCL Stock Down 50% (Benjamin Graham's Secret Model)"* — March 7, 2026. Josh's confirmed March 2026 ORCL entry.
- *"TRUMP State of the Union: Market Reaction LIVE"* — Feb 25, 2026 (macro macro event coverage)
- *"How I Made 30,000% In 90 Seconds"* — March 5, 2026 (options play highlight)
- May 2026 content is behind the $75/month paid membership. Josh posts daily Bullseye Trade alerts on Mondays. Public YouTube uploads for May 2026 are not indexed at time of research.

#### AMD — Updated Thesis Assessment (May 25, 2026)
- AMD consensus PT: **~$405** (34 analysts, majority Buy). Stock may be trading **~$462** — above average consensus.
- Bull case: Evercore ISI $579. Bear case: Morgan Stanley $255.
- DA Davidson upgraded to Buy (citing Intel's structural CPU shift benefiting AMD). Bernstein raised PT (EPYC server CPU sales +50%, Meta AI deal confirmed).
- MI400 platform confirmed: MI455X/Helios (AI superclusters), MI430X (HPC/sovereign AI), MI440X (enterprise) — H2 2026 ramp on track.
- **Assessment: Thesis stronger than expected. Entry timing remains the issue — stock is above entry zone AND above average analyst consensus. No new entries above $420. If in from $390–$400, hold.**

#### Critical NVDA Finding — China Gap
- Q1 FY2027 earnings: Revenue $81.6B (+85% YoY). DC revenue $75.2B (+92%).
- **ZERO China Data Center shipments this quarter vs $4.6B prior year.** Beijing signaled firms to wait on H20 purchases. This is material — China was listed as a key unlocking catalyst.
- Blackwell ramp is fully intact and driving results. China gap does NOT negate the thesis but does delay the China catalyst specifically.
- **Watch signal: Jensen Huang public statements on China deal progress = re-entry signal for China-accelerated position.**

#### LMND — Beat-and-Drop Pattern
- Q1 2026: EPS −$0.47 (beat −$0.57 est.), revenue $258M (+71% YoY). Stock dropped ~15% on earnings day.
- Market is pricing in skepticism of Q4 2026 EBITDA profitability — not questioning the top-line story.
- Morgan Stanley cut PT: $80 → $75. Still Buy.
- **Watch signal: $60–$65 re-test is a better entry zone than current ~$67. Stop at $52 still valid. Q2 August earnings is the credibility test.**

#### SG — Insider Buying Signal
- Stock surged ~45% off May 8–13 base; closed $9.71 on May 22.
- **COO and CFO bought shares near $8 on May 18** — insider buying at the lows. This is a strong thesis-confirmation signal.
- JPMorgan: Overweight, PT $13. Oppenheimer: Outperform, raised PT to $10.
- **Assessment: Insider buying adds conviction to hold through Q2 same-store sales data (July). Thesis unchanged — pure swing on wrap performance.**

#### ORCL — Additional Catalyst Found
- Stock near $192–$193. Wedbush PT $275; Oppenheimer PT $235.
- **New May 2026 catalysts:** US Dept. of War classified AI deployment contract + SoftBank Japan sovereign cloud deal on OCI. Two significant new revenue sources not previously tracked.
- **Assessment: Thesis stronger. These government/sovereign cloud deals are the RPO machine keeping the $523B backlog growing.**

#### MSFT — Thesis Fully Confirmed
- Azure grew 40% in Q3 FY2026 (beat expectations). Copilot paid seats: 20M (up from 15M in January, +250% YoY).
- New: Agent 365 bundle + Azure Foundry Copilot for Factories (Industrial AI, May 13 2026).
- **Assessment: STRONG HOLD. Every data point strengthens the thesis.**

#### New Potential Watch Candidates (Not Yet on List)
| Ticker | Company | Why It Appeared | Status |
|---|---|---|---|
| MU | Micron Technology | +37% YTD; AI infrastructure DRAM/NAND; HBM memory for AI training — limited alternatives; pricing power via supply constraints | **Watch — potential addition** |
| EATON | Eaton Corporation | AI buildout power infrastructure; UBS-backed; CNBC flagged May 13, 2026 | Watch |
| INTC | Intel | +197% YTD; "changing of the guard" CNBC May 8 — but RSI extended | Archived; re-entry at $95–$105 only |
| SanDisk | SanDisk | +464% YTD — extreme momentum; high risk | Do not chase |

**MU (Micron) is the strongest new candidate.** Passes pricing power filter (HBM memory for AI training has limited supply alternatives). Catalyst: AI data center demand for both DRAM and NAND. Macro: AI infrastructure supercycle tailwind. Will run through full framework on next "call up" and evaluate for watchlist v4 addition.

---

### Session Notes — Watchlist v3 + Live Stream Notes — May 25, 2026
- **Watchlist grown to 7:** Added MSFT (AI Backbone, Tier 1) and NVDA (AI Infrastructure, Tier 2)
- **MSFT thesis:** Azure +40% YoY, 20M Copilot seats at $30/seat/mo — pricing power maximum. Both Kevin and Josh frameworks score max conviction. July Q4 FY2026 earnings is the next hard catalyst date. Wedbush PT $625, Morgan Stanley PT $650.
- **NVDA thesis:** YTD flat (+15%) was the opportunity — China export restriction removal unlocks $10B+ revenue. Blackwell GB300 ramp in Q3 2026 is the hardware catalyst. Jensen Beijing visit confirms deal momentum. Unusual call premium noted CNBC May 20, 2026 = institutional tell.
- **Conviction ranking established:** MSFT > NVDA > ORCL > AMD > LMND > TAP > SG
- **app.py synced:** WATCHLIST array in Flask backend updated from v1 (5 stocks, wrong tickers) to v3 (7 stocks, correct tickers including LMND swap-out of INTC)
- **Live stream notes created:** `/home/user/Test-/notes/live_stream_notes.md` — "Powell Pre-Game" stream identified; 3 context-based takeaways documented (Fed day trading approach, INTC vs AMD setup, TAP ex-div window). Note: direct stream content unavailable from cloud environment (YouTube 403); transcript is context-inferred from Josh's documented framework.
- **Gap (live stream):** Direct YouTube stream fetch blocked — recommend using browser to access youtube.com/@StockMarketLive directly; transcripts available via "Open transcript" after stream ends.
- **"Call up" trigger:** Now sweeps Trading Fraternity, StockMarketLive, AND Meet Kevin channels. Adds MSFT and NVDA to the refresh queue.

### Session Notes — Evolution Update May 24, 2026 (Post-Kevin Research)
- **v2 Evolution complete**: Integrated Meet Kevin's framework alongside Josh's. Pricing Power filter now applied before any new watchlist entry.
- **Philosophy shift**: Single-source → Dual-source synthesis. Josh = discipline engine. Kevin = idea filter.
- **Watchlist v2**: ORCL (elevated), AMD (elevated to primary), LMND (added), TAP (reduced conviction), SG (revised thesis), INTC (archived)
- **Critical find**: Sweetgreen sold Infinite Kitchen tech to Wonder (Nov 2025) — AI moat removed from SG thesis. This changed the conviction level and time horizon.
- **Kevin blind spot confirmed**: 33% tracked pick accuracy; his ETF (PP) closed Feb 2025. Use his *framework*, ignore his *specific blind calls* (HUT, Shift).
- **Institutional sources added**: Wedbush, Morgan Stanley, J.P. Morgan, TrueAlpha Data
- **"Call up" trigger updated** — now sweeps both Trading Fraternity AND Meet Kevin channels for recent content
- Gap: Cannot access Josh's paid stream alerts ($75/mo) — member-only watchlists not public
- Gap: Cannot access Kevin's paid course content — framework extracted from public ETF filings + YouTube research
- Gap: "3 Stocks for 2026" from Josh's Feb 2, 2026 video — tickers still unconfirmed (URL known: watch?v=FSFLgln8cxQ)
- Gap: CMG (Chipotle) current 2026 performance not yet pulled
- Queue on next "call up": Pull May/June uploads from both channels; confirm LMND Q2 guidance; check AMD MI400 shipping status; re-evaluate INTC RSI for re-entry

---

*Last updated: May 24, 2026 | Sources: The Trading Fraternity YouTube, StockMarketLive, X/@JoshAnswers, New York Weekly, public market data*

*Everything in this document is educational content in the spirit of Josh Answers' documented teaching methodology. Not personalized financial advice. Do your own due diligence.*
