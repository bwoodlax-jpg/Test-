...
WATCHLIST = [
    {"ticker": "MSFT", "company": "Microsoft Corporation",    "tier": "AI Backbone",               "tier_class": "t1", "entry": "$393–$412", "target": "$625", "stop": "$372",  "catalyst": "Azure +40% YoY; 20M Copilot seats; Wedbush PT $625; Morgan Stanley PT $650", "note": "🔴 RED ALERT Jun 27: $379 Jun 18 = $7 above stop. Stand-down in effect; zone $393–$412 not reached. Stop $372 = hard floor. Iran deal + Warsh hold = framework gates cleared, but price must reclaim zone."},
    ...
    {"ticker": "ORCL", "company": "Oracle Corporation",       "tier": "Long-Term Hold",            "tier_class": "t1", "entry": "$165–$182", "target": "$275", "stop": "$148",  "catalyst": "AI cloud RPO $523B; Wedbush PT $275; beat-but-fell Jun 10 driving toward zone", "note": "🟢 PRIMARY SWING JUN 30–JUL 4: $184 Jun 18, alert $183. Entry trigger if touches $182 or below Jun 22–27. Zone = Graham value + DB lock-in. 1% bankroll size. Stop $148 (structural floor). R/R 4.0:1 zone mid."},
    ...
    {"ticker": "MRVL", "company": "Marvell Technology Inc",    "tier": "AI Custom Silicon",         "tier_class": "t2", "entry": "$220–$242", "target": "$350", "stop": "$198→trail $290",  "catalyst": "AWS Trainium3 + Google TPU contracts; S&P 500 inclusion Jun 22; AI ASIC rev +87% YoY", "note": "🟡 POST-INCLUSION SWING JUN 30–JUL 4: $311 Jun 18 (+18% from $263 entry). Trail stop raised $263→$290 Jun 22. If held $290 through Jun 27: post-inclusion week is secondary test. Trim 50% if $305+, hold 50% if AI trend intact. No weekend holds."},
    ...
    {"ticker": "NOW",  "company": "ServiceNow Inc",             "tier": "AI Enterprise Control Plane","tier_class": "t1", "entry": "$103–$118", "target": "$175", "stop": "$88",   "catalyst": "Knowledge 2026: AI Control Tower + Otto agent; Experian multi-year agentic deal; consensus PT $142; 19/22 analysts Buy", "note": "🔴 STAND-DOWN JUN 27: $95 Jun 18, zone floor $103 = $8 below entry. Thesis intact (Otto AI agent + Experian deal). Wait for $103+ reclaim before entry. Stop $88 = hard floor. Iran deal removed macro drag; framework gates cleared. Re-entry trigger: $103 sustained."},
]
